...
5. [终端功能](#终端功能)
6. [串口终端](#串口终端)
7. [SFTP 文件管理](#sftp-文件管理)
8. [端口转发](#端口转发)
9. [扩展能力（插件 / 图形 / 主题）](#user-guide-extensions)
10. [高级功能](#高级功能)
11. [快捷键速查](#快捷键速查)
12. [最佳实践](#最佳实践)
13. [故障排查](#故障排查)
...
串口终端是本地设备传输，不是 SSH 子功能。它适用于开发板、路由器控制台、交换机串口控制台、USB-UART 适配器等场景。

### 打开方式

- 新建连接弹窗顶部选择 **Serial**，填写 **Serial port**、**Baud rate**、**Data bits**、**Stop bits**、**Parity** 和 **Flow control**，然后点击 **Open Serial**。
- 命令面板中选择 **Open Serial Terminal**。
- 如需复用配置，勾选 **Save serial profile** 并填写 **Profile name**。保存后会出现在连接管理器的串口配置列表中。
...
- 串口终端只打开本地串口设备并把字节流接入终端界面。
- 串口终端不提供 SFTP、端口转发、ProxyJump、SSH 主机密钥校验、SSH Agent、远程 IDE 或 SSH 连接池能力。
- 串口终端默认不支持分屏。同一个串口设备通常是独占资源，同时打开多个写入端容易造成输入交错或设备占用错误。
- AI 上下文会标记为 `serial`，SSH-only 工具会拒绝在串口目标上执行。

### 平台权限

- macOS：优先使用 `/dev/cu.*` 设备名。首次连接可能触发系统权限提示，USB 转串口驱动也可能需要单独安装。
- Linux：普通用户可能没有串口权限。常见做法是把用户加入 `dialout`、`uucp` 或发行版对应的串口组，然后重新登录。不要长期用 `sudo` 启动桌面应用来规避权限。
- Windows：使用设备管理器确认实际 COM 端口号。高编号端口仍按界面显示的 `COMx` 填写即可。

### 常见问题

- **No serial ports found**：可以手动输入端口路径；同时检查设备是否插入、驱动是否安装、系统是否能看到该串口。
- **Permission denied**：检查 Linux 串口组权限或 macOS 系统权限；修改组权限后需要退出登录再进入。
- **Device busy**：该串口已被 OxideTerm 的另一个标签页、其他终端程序、调试器或烧录工具占用。关闭占用者后重试。
- **设备拔出后终端报错**：这是预期行为。重新插入后，使用相同端口名重新打开；如果系统分配了新端口名，需要在串口配置里改成新路径。
...
用于在设备间迁移连接、转发、应用设置、快捷命令、插件设置以及可选凭据材料。`.oxide` 文件本身使用 ChaCha20-Poly1305 和 Argon2id 加密；只要导出时包含密码、私钥、托管密钥或便携凭据，就应按高敏感备份文件处理。

导出时凭据材料是显式选项：

- 保存的服务器密码默认不包含。
- 外部私钥文件默认不嵌入；选择嵌入后会复制进加密 `.oxide`。
- 外部密钥/证书的已保存 passphrase 会作为单独选项显示。
- OxideTerm 托管密钥会以托管密钥语义写入加密 `.oxide`，导入时可恢复到本地托管密钥库。
- 托管密钥 passphrase 默认不包含，便携迁移场景可显式包含。
- 便携凭据（例如 AI provider key）只在便携迁移或用户显式选择时包含。

导入预览会显示连接、转发、设置、插件设置、快捷命令、便携凭据和托管密钥。托管密钥可以恢复到 OxideTerm 托管密钥库；如果选择不恢复，连接可能需要重新配置认证或使用嵌入 fallback 密钥文件。

**WebDAV / Cloud Sync 边界**：Cloud Sync 使用 `.oxide` 快照作为同步载体，但后台同步默认不静默包含托管密钥等秘密材料。需要跨机器迁移完整凭据时，使用桌面端手动导出或便携迁移流程，并确认导出选项。

**操作建议**：使用足够长的密码；含凭据材料的 `.oxide` 文件视为高敏感；导入前先看预览；只在确实需要完整迁移时包含秘密材料。
...
- **没有检测到串口**：手动输入 `/dev/cu.*`、`/dev/ttyUSB*`、`/dev/ttyACM*` 或 `COMx`；确认设备、线缆、驱动和系统权限。
- **权限被拒绝**：Linux 检查 `dialout`/`uucp` 等串口组；macOS 检查系统权限和驱动；修改权限后重新登录或重启应用。
- **设备忙**：关闭占用该端口的其他程序或 OxideTerm 标签页。
- **拔出后无法继续输入**：关闭当前串口终端，重新插入设备后按新的端口名重新打开。
